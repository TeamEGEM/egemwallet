<main class="tab-pane active" ng-if="globalService.currentTab==globalService.tabs.addWallet.id" ng-controller='addWalletCtrl'  ng-cloak>
  ...
  <section class="row" id="walletselection">
    ...
    <article class="col-md-4 col-sm-6">
      ...
      <div ng-show="walletType=='watchonlyaccount'">
        ...
        <p class="text-danger" translate="ADD_Warning_1">You can add any account to "watch" on the wallets tab without uploading a private key. This does ** not ** mean you have access to this wallet, nor can you transfer Ethergem from it. </p>
        <div class="form-group">
          <label translate="ADD_Label_2"> Create a Nickname: </label>
  ...
  <article class="row" ng-if="showAddWallet">
    ...
    <div class="col-sm-4">
      ...
      <ul class="account-info">
        <li><strong>{{etherBalance}}</strong> EGEM </li>
      </ul>
      ...
      <ul class="account-info">
        <li><a href="https://explorer.egem.io/addr/{{wallet.getAddressString()}}" target="_blank" rel="noopener">https://explorer.egem.io/addr/{{wallet.getAddressString()}}</a></li>
      </ul>
    </div>
