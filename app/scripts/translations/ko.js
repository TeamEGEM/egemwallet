...
x_JsonDesc                  : '이것은 암호화되지 않은 JSON 형식의 개인 키입니다. 이 암호화되지 않은 JSON 형식의 개인 키를 사용하면 누구나 암호없이 자신의 지갑과 엘라리즘에 액세스 할 수 있습니다. ',
...
/* Header */
CX_Tagline                  : '오픈 소스 JavaScript 클라이언트 측 엘라리즘 지갑 Chrome 확장 프로그램 ',
...
MEW_Tagline                 : '오픈 소스 JavaScript 클라이언트 측 엘라리즘 지갑 ',
MEW_Warning_1               : '지갑에 액세스하거나 새 지갑을 생성하기 전 반드시 URL을 확인해주세요. 피싱(사기) 사이트에 주의하세요! ',

/* Footer */
FOOTER_1                    : '쉽게 사용할 수 있는 오픈 소스, 클라이언트 측 도구 & 엘라리즘 네트워크와 안전하게 상호 작용합니다. ',
FOOTER_1b                   : '개발자 ',
...
ADD_Ledger_2                : '엘라리즘 어플리케이션을 실행해주세요 (또는 컨트랙트 어플리케이션)  ',
...
ADD_Warning_1               : '지갑 탭은 개인 키를 업로드하지 않고 임의로 모니터링하는 계좌를 추가할 수 있습니다. 이에 따라 지갑의 액세스나 엘라리즘 전송이 가능하지 않습니다. ',
ADD_Label_5                 : '주소 입력 ',
...
SENDModal_Content_4         : '참고: 에러가 발생한다면, 대부분 토큰 전송에 필요한 가스(수수료, 엘라리즘으로 결제)가 부족한 것입니다. 엘라리즘을 계좌에 추가해 주세요. ',
...
TRANS_warning               : '"엘라리즘만 전송하기" 또는 "이더리움 클래식만 전송하기" 를 사용하여 전송할 때, 몇 가지 서비스는 컨트랙트가 체결되지 않을 수 있습니다. 더보기. ',
...
HELP_Remind_Desc_1          : '**엘라리즘, MyEtherWallet.com & MyEtherWallet CX, 그리고 우리가 사용하는 JavaScript 라이브러리는 활발하게 개발되고 있습니다. ** 우리가 철저하게 테스트를 진행하여 수만 개의 지갑이 전 세계 사람들에 의해 성공적으로 만들어졌지만 예상치 못한 일이 발생하여 ETH를 잃을 가능성이 항상 존재합니다. 항상 조심해주세요. 만약 무슨 일이 발생한다면, 유감스럽지만 **우리는 잃어버린 엘라리즘에 대한 책임을 지지 않습니다.** ',
HELP_Remind_Desc_2          : 'MyEtherWallet.com 과 MyEtherWallet CX는 웹 지갑이 아닙니다. 계정을 만들거나 우리에게 당신이 가진 엘라리즘을 주면 안 됩니다. 모든 데이터는 당신의 컴퓨터와 브라우저에 남아 있을 것입니다. 우리는 당신이 쉽게 당신의 정보를 만들고, 저장하고, 접근하고 블록체인과 상호 작용할 수 있도록 도울 것입니다. ',
...
HELP_0_Desc_1               : 'MyEtherWallet은 새로운 지갑을 생성할 수 있는 기능을 제공하므로 거래소에 엘라리즘을 두지 않고 직접 저장할 수 있습니다. 이 과정은 전적으로 당신의 컴퓨터에서 발생하고 우리의 서버에서 발생하지 않습니다. 따라서 새로운 지갑을 만들 때 **안전하게 백업’해야 합니다**. ',
HELP_0_Desc_2               : '새로운 지갑 생성하기 ',
HELP_0_Desc_3               : '지갑 백업하기 ',
HELP_0_Desc_4               : '새로운 지갑에 대한 액세스 권한과 필요한 모든 정보를 올바르게 저장하였는지 확인해주세요. ',
HELP_0_Desc_5               : '엘라리즘을 새로운 지갑으로 전송해보세요. ',
...
HELP_2a_Desc_2              : '주소를 저장해주세요. 주소는 자신만 확인할 수 있도록 보관하거나 다른 사람에게 공유할 수 있습니다. 주소를 공유하면 다른 사람이 해당 주소로 엘라리즘을 전송할 수 있습니다. ',
HELP_2a_Desc_3              : '개인 키를 저장해주세요. 절대 타인과 공유하지 마세요. 개인 키는 지갑에 액세스하거나 엘라리즘을 전송하는 데 필요합니다! 개인 키에는 3가지 유형이 있습니다. ',
...
HELP_3_Desc_1               : '**새 지갑에 엘라리즘을 전송하기 전**에 지갑에 접근할 수 있는지 확인해야 합니다. ',
...
HELP_3_Desc_8               : '많은 양의 엘라리즘을 MyEtherWallet 으로 전송하기 전에, 소량의 엘라리즘을 전송해 보는 것을 권장합니다. 0.001 엘라리즘을 지갑으로 전송하고, 지갑을 열어 받은 엘라리즘을 확인하고, 다시 0.001 엘라리즘을 다른 주소로 전송하여 모든 과정이 잘 작동하는지 확인해주세요. ',

HELP_4_Title                : '4) 한 지갑에서 다른 지갑으로 엘라리즘을 어떻게 전송하나요? ',
HELP_4_Desc_1               : '많은 양의 엘라리즘을 다른 지갑으로 전송하기 전에 소량의 엘라리즘을 전송하여 전송이 잘 되는지 확인해주세요. ',
HELP_4_Desc_2               : '"엘라리즘 & 토큰 전송하기" 페이지로 이동합니다. ',
...
HELP_4_Desc_6               : '"받는 주소" 영역에 엘라리즘을 전송할 주소를 입력합니다. ',
...
HELP_4CX_Title              : '4) 엘라리즘을 MyEtherWallet CX를 사용하여 전송하는 방법이 궁급합니다. ',
HELP_4CX_Desc_1             : '우선 지갑을 추가해야 합니다. 그 후에, 2가지 방법이 있습니다. "빠른 전송" 또는 "엘라리즘 & 토큰 전송" 페이지입니다. ',
...
HELP_4CX_Desc_6             : '"받는 주소" 영역에 전송한 엘라리즘을 받게 될 주소를 입력합니다. ',
...
HELP_4CX_Desc_12            : '"엘라리즘 & 토큰 전송하기" 페이지를 사용하는 방법',
...
HELP_7_Desc_1               : '엘라리즘 & 토큰 전송 페이지로 이동합니다. ',
...
HELP_8_Desc_2               : '만약 MyEtherWallet.com 이 다운된다면, 다른 대안 도구 (geth 또는 Ethereum Wallet / Mist 등)을 사용해야 합니다. MyEtherWallet 에서 엘라리즘을 꺼내야 할 필요는 없습니다. 엘라리즘들은 MyEtherWallet.com 을 통해서 생성된 지갑 내에 존재합니다. ',
HELP_8_Desc_3               : '암호화되지 않은 개인 키와 암호화된 Geth/Mist 형식의 파일은 쉽게 geth / 엘라리즘 지갑 / Mist에서 불러올 수 있습니다. 아래의 질문 #12를 확인해주세요. ',
...
HELP_9_Title                : '9) "엘라리즘 & 토큰 전송하기" 페이지는 오프라인인가요? ',
...
HELP_12_Title               : '12) MyEtherWallet으로 만든 지갑을 geth / 엘라리즘 지갑 / Mist로 가져오려면 어떻게 해야 하나요? ',
...
HELP_12_Desc_7              : '엘라리즘 어플리케이션을 실행합니다. ',
...
HELP_12_Desc_22             : '이후 엘라리즘 지갑 어플리케이션을 열 면 계정이 "계정" 아래에 표시됩니다.". ',
...
HELP_12_Desc_26             : '명령을 실행하면 새 비밀번호를 입력하라는 메시지가 나타납니다. 이것은 트랜잭션을 전송할 때마다 geth / 엘라리즘 지갑 / Mist에서 사용할 비밀번호이므로 절대 잃어버리지 마십시오. ',
HELP_12_Desc_27             : '가져오기가 성공하면 "nothing_special_delete_me.txt"를 삭제합니다. ',
HELP_12_Desc_28             : '다음에 엘라리즘 지갑 어플리케이션을 열 면 계정이 "계정" 아래에 표시됩니다. ',

HELP_13_Title               : '13) "잔액 부족. 계좌에 충분한 자금이 없습니다. 필요한 : XXXXXXXXXXXXXXXXXXX 그리고 가지고 있는: XXXXXXXXXXXXXXXXXXX" 는 어떤 의미인가요? ',
HELP_13_Desc_1              : '이것은 가스 비용을 충당하기 위해 계좌에 엘라리즘이 부족하다는 의미입니다. 각각의 트랜잭션(토큰 및 컨트랙트 트랜잭션을 포함하여) 가스가 필요하고 그 가스는 엘라리즘으로 지불됩니다. 표시된 숫자는 Wei에서 트랜잭션의 비용을 충당하기 위해 필요한 금액입니다. 이 숫자를 "1000000000000000000"으로 나눈 다음, 당신이 전송하려고 하는 엘라리즘의 금액을 제외합니다.(엘라리즘을 전송하려고 하는 경우) 이렇게 하면 트랜잭션을 만들기 위해 해당 계좌로 전송하는데 필요한 엘라리즘의 금액을 알 수 있습니다. ',
...
HELP_15_Desc_1              : '계좌는 거래 활동이 있을 경우에만 블록체인 탐색기에 표시됩니다. 예를 들어, 엘라리즘을 계좌로 이동시키면 계좌가 활성화됩니다. ',
...
HELP_21_Desc_2              : '길지만, 최대한 쉽게 설명한 답변: 그래서 엘라리즘은 [공개 키 암호화](https://en.wikipedia.org/wiki/Public-key_cryptograpy) 특히나 [타원곡선 암호화](https://eprint.iacr.org/2013/734.pdf) 를 기반으로 합니다. 대부분의 서버는 ECC를 통해 보호됩니다. 비트코인은 SSH와 TLS 뿐만 아니라 다른 것들도 많이 사용합니다. 엘라리즘의 키는 128bit와 192bit보다 더 강력한 256bit 키이며, 이것들은 널리 사용되고 있으며 전문가들도 안전하다고 얘기합니다. ',
...
HELP_21_Desc_4              : '만약 누군가가 당신의 개인 키를 소지하고 있다면 당신의 계좌에서 엘라리즘을 보낼 수 있게 됩니다. 이 얘기는 누군가가 당신의 이메일의 비밀번호를 알고 있다면 당신의 이메일을 읽을 수 있으며, 이메일이나 누군가가 출금할 수 있도록 당신의 은행 계좌 비밀번호를 전송할 수 있다는 것과 마찬가지입니다. 당신은 비밀번호로 암호화된 개인 키인 ‘개인 키의 Keystore 버전’을 다운로드 할 수 있습니다. 이것은 다른 암호로 보호되는 비밀번호를 갖게 되는 것입니다. ',
